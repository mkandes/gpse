...
!        compiler and system runtime environment.
!
!     REAL_128 is a PUBLIC, INTEGER-valued PARAMETER that stores the
!        KIND parameter for 128-bit REALs, if they are supported by the 
!        compiler and system runtime environment.
!
!      CMPLX_DEFAULT_KIND is a PUBLIC, INTEGER-valued PARAMETER that
!         stores the KIND parameter for COMPLEXs at runtime. It should
...
!
!     alaguerre is a PUBLIC, REAL scalar function that recursively
!        computes the value of the associated Laguerre polynomial of 
!        degree n and order k at the point x.
!
!     hermite is a PUBLIC, REAL scalar function that recursively
!        computes the value of the Hermite polynomial of degree n at the 
!        point x.
!
!     laguerre is a PUBLIC, REAL scalar function that recursively
!        computes the value of the Laguerre polynomial of degree n at 
!        the point x.
!
...
! ----------------------------------------------------------------------
      ...
      IF (KIND(n) == INT_32) THEN
         ...
         ELSE
            ...
            nFactorial = -1
            WRITE(UNIT=ERROR_UNIT, FMT=*) 'math: factorial:: ERROR - &
               & n must be an integer greater than or equal to 0, but &
               & less than or equal to 12 because KIND(n) = INT32.'
            STOP
      ...
      ELSE IF (KIND(n) == INT_64) THEN
         ...
         ELSE
            ...
            WRITE(UNIT=ERROR_UNIT, FMT=*) 'math: factorial:: ERROR - &
               & n must be an integer greater than or equal to 0, but &
               & less than or equal to 20 because KIND ( n ) = INT64.'
            STOP
...
! ----------------------------------------------------------------------
      ...
      IF (k > -1) THEN
         ...
         ELSE IF (n >= 2) THEN
            ...
            alaguerreNK = ((2.0 * REAL(n) + REAL(k) - 1.0 - x) * &
               & alaguerre(n - 1, k, x) - (REAL(n) + REAL(k) - 1.0) * &
               & alaguerre(n - 2, k, x)) / REAL(n)

         ELSE

            WRITE(UNIT=ERROR_UNIT, FMT=*) 'math: alaguerre:: ERROR - &
               n must be a non-negative integer greater than 0.'
            STOP
...
! ----------------------------------------------------------------------
      ...
      ELSE IF (n >= 2) THEN
         ...
         hermiteN = 2.0 * x * hermite(n - 1, x) - &
            & 2.0 * REAL(n - 1) * hermite(n - 2, x)    

      ELSE

         WRITE(UNIT=ERROR_UNIT, FMT=*) 'math: hermite:: ERROR - &
               n must be a non-negative integer greater than 0.'
         STOP
...
! ----------------------------------------------------------------------
      ...
      ELSE
         ...
         WRITE(UNIT=ERROR_UNIT, FMT=*) 'math: laguerre:: ERROR - &
            n must be a non-negative integer greater than 0.'
         STOP
