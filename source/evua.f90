...
!
!     evuaL2norm is a REAL, PRIVATE variable used to store the L^2-norm of 
...
!        respect to the condensate's center of mass.
!
!     evuaLx2a is a REAL, PRIVATE variable used to store the condensate's
!        average squared angular momentum about the X-axis of the system
!        with respect to the origin of the computational domain.
!
!     evuaLx2b is a REAL, PRIVATE variable used to store the condensate's
!        average squared angular momentum about the X-axis of the system
!        with respect to the condensate's center of mass.
!
!     evuaLy2a is a REAL, PRIVATE variable used to store the condensate's
!        average squared angular momentum about the Y-axis of the system
!        with respect to the origin of the computational domain.
!
!     evuaLy2b is a REAL, PRIVATE variable used to store the condensate's
!        average squared angular momentum about the Y-axis of the system
!        with respect to the condensate's center of mass.
!
!     evuaLz2a is a REAL, PRIVATE variable used to store the condensate's
!        average squared angular momentum about the Z-axis of the system
!        with respect to the origin of the computational domain.
!
!     evuaLz2b is a REAL, PRIVATE variable used to store the condensate's
!        average squared angular momentum about the Z-axis of the system
!        with respect to the condensate's center of mass.
!
...
!        average total energy measured with respect to the origin of the
!        computational domain.
!
!     evuaEb is a REAL, PRIVATE variable used to store the condensate's
!        average total energy measured with respect to its center of mass.
!
...
!        computational domain.
!
!     evuaSigXa is a REAL, PRIVATE variable used to store the condensate's
!        position uncertainty along the X-axis of the system and measured 
!        with respect to the origin of the computational domain.
!
!     evuaSigXb is a REAL, PRIVATE variable used to store the condensate's
!        position uncertainty along the X-axis of the system and measured 
!        with respect to the condensate's center of mass.
!
!     evuaSigYa is a REAL, PRIVATE variable used to store the condensate's
!        position uncertainty along the Y-axis of the system and measured 
!        with respect to the origin of the computational domain.
!
!     evuaSigYb is a REAL, PRIVATE variable used to store the condensate's
!        position uncertainty along the Y-axis of the system and measured 
!        with respect to the condensate's center of mass.
!
!     evuaSigZa is a REAL, PRIVATE variable used to store the condensate's
!        position uncertainty along the Z-axis of the system and measured 
!        with respect to the origin of the computational domain.
!
!     evuaSigZb is a REAL, PRIVATE variable used to store the condensate's
!        position uncertainty along the Z-axis of the system and measured 
!        with respect to the condensate's center of mass.
!
!     evuaSigPx is a REAL, PRIVATE variable used to store the condensate's
!        momentum uncertainty along the X-axis of the system.
!
!     evuaSigPy is a REAL, PRIVATE variable used to store the condensate's
!        momentum uncertainty along the Y-axis of the system.
!
!     evuaSigPz is a REAL, PRIVATE variable used to store the condensate's
!        momentum uncertainty along the Z-axis of the system.
!
!     evuaSigLxA is a REAL, PRIVATE variable used to store the uncertainty
!        in the angular momentum of the condensate about the X-axis of the
!        system and measured with respect to the origin of the
!        computational domain.
!
!     evuaSigLxB is a REAL, PRIVATE variable used to store the uncertainty
!        in the angular momentum of the condensate about the X-axis of the
!        system and measured with respect to the condensate's center of 
!        mass.
!
!     evuaSigLyA is a REAL, PRIVATE variable used to store the uncertainty
!        in the angular momentum of the condensate about the Y-axis of the
!        system and measured with respect to the origin of the
!        computational domain.
!
!     evuaSigLyB is a REAL, PRIVATE variable used to store the uncertainty
!        in the angular momentum of the condensate about the Y-axis of the
!        system and measured with respect to the condensate's center of 
!        mass.
!
!     evuaSigLzA is a REAL, PRIVATE variable used to store the uncertainty
!        in the angular momentum of the condensate about the Z-axis of the
!        system and measured with respect to the origin of the
!        computational domain.
!
!     evuaSigLzB is a REAL, PRIVATE variable used to store the uncertainty
!        in the angular momentum of the condensate about the Z-axis of the
!        system and measured with respect to the condensate's center of
!        mass.
!
...
!
!     evua_compute_base is a PUBLIC SUBROUTINE that calls functions which 
!        computes base quantities associated with a wave function.
!
!     evua_compute_derived is a PUBLIC SUBROUTINE that computes quantities 
!        which are derived from base quantities associated with a wave
!        function.
!
!     evua_write_all is a PUBLIC SUBROUTINE that formats and writes all 
!        quantities, both base and derived, to standard output.
!
!     evua_normalize is a PULIC SUBROUTINE that may be used to normalize a
!        wave function. It is currently used only to re-normalize the wave 
!        function after every time step when performing imaginary time 
!        propagation.
!
...
!
!     evua_l2_norm_3d_rect is a PRIVATE, REAL scalar function that computes
!        the L^2-norm of a three-dimensional wave function defined on a
!        regular Cartesian grid using the rectangle rule.
!
!     evua_x_3d_rect is a PRIVATE, REAL scalar function that computes the
!        x-component of the position expectation value for a 
!        three-dimensional wave function defined on a regular Cartesian 
!        grid using the rectangle rule. 
!
!     evua_y_3d_rect is a PRIVATE, REAL scalar function that computes the
!        y-component of the position expectation value for a 
!        three-dimensional wave function defined on a regular Cartesian 
!        grid using the rectangle rule. 
!
!     evua_z_3d_rect is a PRIVATE, REAL scalar function that computes the
!        z-component of the position expectation value for a 
!        three-dimensional wave function defined on a regular Cartesian 
!        grid using the rectangle rule. 
!
!     evua_r_xy_3d_rect is a PRIVATE, REAL scalar function that computes
!        the planar (xy-plane) radial position expectation value for a 
!        three-dimensinal wave function defined on a regular Cartesian 
!        grid using the rectangle rule.
!
!     evua_r_xyz_3d_rect is a PRIVATE, REAL scalar function that computes
!        the spherical radial position expectation value for a 
!        three-dimensional wave function defined on a regular Cartesian
!        grid using the rectangle rule.
!
!     evua_x2_3d_rect is a PRIVATE, REAL scalar function that computes the
!        x-component of the squared position expectation value for a
!        three-dimensional wave function defined on a regular Cartesian
!        grid using the rectangle rule.
!
!     evua_y2_3d_rect is a PRIVATE, REAL scalar function that computes the
!        y-component of the squared position expectation value for a
!        three-dimensional wave function defined on a regular Cartesian
!        grid using the rectangle rule.
!
!     evua_z2_3d_rect is a PRIVATE, REAL scalar function that computes the
!        z-component of the squared position expectation value for a
!        three-dimensional wave function defined on a regular Cartesian
!        grid using the rectangle rule.
!
!     evua_r2_xy_3d_rect is a PRIVATE, REAL scalar function that computes
!        the squared planar (xy-plane) radial position expectation value
!        for a three-dimensional wave function defined on a regular
!        Cartesian grid using the rectangle rule.
!
!     evua_r2_xyz_3d_rect is a PRIVATE, REAL scalar function that computes
!        the squared spherical radial position expectation value for a
!        three-dimensional wave function defined on a regular Cartesian
!        grid using the rectangle rule.
!
!     evua_px_3d_rect_cd2 is a PRIVATE, REAL scalar function that computes
!        the x-component of the momentum expectation value for a
!        three-dimensional wave function defined on a regular Cartesian
!        grid using the rectangle rule and 2nd-order central differences.
!
!     evua_px_3d_rect_cd4 is a PRIVATE, REAL scalar function that computes
!        the x-component of the momentum expectation value for a
!        three-dimensional wave function defined on a regular Cartesian
!        grid using the rectangle rule and 4th-order central differences.
!
!     evua_py_3d_rect_cd2 is a PRIVATE, REAL scalar function that computes
!        the y-component of the momentum expectation value for a
!        three-dimensional wave function defined on a regular Cartesian
!        grid using the rectangle rule and 2nd-order central differences.
!
!     evua_py_3d_rect_cd4 is a PRIVATE, REAL scalar function that computes
!        the y-component of the momentum expectation value for a
!        three-dimensional wave function defined on a regular Cartesian
!        grid using the rectangle rule and 4th-order central differences.
!
!     evua_pz_3d_rect_cd2 is a PRIVATE, REAL scalar function that computes
!        the z-component of the momentum expectation value for a
!        three-dimensional wave function defined on a regular Cartesian
!        grid using the rectangle rule and 2nd-order central differences.
!
!     evua_pz_3d_rect_cd4 is a PRIVATE, REAL scalar function that computes
!        the z-component of the momentum expectation value for a
!        three-dimensional wave function defined on a regular Cartesian
!        grid using the rectangle rule and 4th-order central differences.
!
!     evua_px2_3d_rect_cd2 is a PRIVATE, REAL scalar function that computes
!        the x-component of the squared momentum expectation value for a
!        three-dimensional wave function defined on a regular Cartesian
!        grid using the rectangle rule and 2nd-order central differences.
!
!     evua_px2_3d_rect_cd4 is a PRIVATE, REAL scalar function that computes
!        the x-component of the squared momentum expectation value for a
!        three-dimensional wave function defined on a regular Cartesian
!        grid using the rectangle rule and 4th-order central differences.
!
!     evua_py2_3d_rect_cd2 is a PRIVATE, REAL scalar function that computes
!        the y-component of the squared momentum expectation value for a
!        three-dimensional wave function defined on a regular Cartesian
!        grid using the rectangle rule and 2nd-order central differences.
!
!     evua_py2_3d_rect_cd4 is a PRIVATE, REAL scalar function that computes
!        the y-component of the squared momentum expectation value for a
!        three-dimensional wave function defined on a regular Cartesian
!        grid using the rectangle rule and 4th-order central differences.
!
!     evua_pz2_3d_rect_cd2 is a PRIVATE, REAL scalar function that computes
!        the z-component of the squared momentum expectation value for a
!        three-dimensional wave function defined on a regular Cartesian
!        grid using the rectangle rule and 2nd-order central differences.
!
!     evua_pz2_3d_rect_cd4 is a PRIVATE, REAL scalar function that computes
!        the z-component of the squared momentum expectation value for a
!        three-dimensional wave function defined on a regular Cartesian
!        grid using the rectangle rule and 4th-order central differences.
!
!     evua_lx_3d_rect_cd2 is a PRIVATE, REAL scalar function that computes
!        the x-component of the angular momentum expectation value for a
!        three-dimensional wave function defined on a regular Cartesian
!        grid using the rectangle rule and 2nd-order central differences.
!
!     evua_lx_3d_rect_cd4 is a PRIVATE, REAL scalar function that computes
!        the x-component of the angular momentum expectation value for a
!        three-dimensional wave function defined on a regular Cartesian
!        grid using the rectangle rule and 4th-order central differences.
!
!     evua_ly_3d_rect_cd2 is a PRIVATE, REAL scalar function that computes
!        the y-component of the angular momentum expectation value for a
!        three-dimensional wave function defined on a regular Cartesian
!        grid using the rectangle rule and 2nd-order central differences.
!
!     evua_ly_3d_rect_cd4 is a PRIVATE, REAL scalar function that computes
!        the y-component of the angular momentum expectation value for a
!        three-dimensional wave function defined on a regular Cartesian
!        grid using the rectangle rule and 4th-order central differences.
!
!     evua_lz_3d_rect_cd2 is a PRIVATE, REAL scalar function that computes
!        the z-component of the angular momentum expectation value for a
!        three-dimensional wave function defined on a regular Cartesian
!        grid using the rectangle rule and 2nd-order central differences.
!
!     evua_lz_3d_rect_cd4 is a PRIVATE, REAL scalar function that computes
!        the z-component of the angular momentum expectation value for a
!        three-dimensional wave function defined on a regular Cartesian
!        grid using the rectangle rule and 4th-order central differences.
!
!     evua_lx2_3d_rect_cd2 is a PRIVATE, REAL scalar function that computes
!        the x-component of the squared angular momentum expectation value
!        for a three-dimensional wave function defined on a regular 
!        Cartesian grid using the rectangle rule and 2nd-order central
!        differences.
!
!     evua_lx2_3d_rect_cd4 is a PRIVATE, REAL scalar function that computes
!        the x-component of the squared angular momentum expectation value
!        for a three-dimensional wave function defined on a regular 
!        Cartesian grid using the rectangle rule and 4th-order central
!        differences.
!
!     evua_ly2_3d_rect_cd2 is a PRIVATE, REAL scalar function that computes
!        the y-component of the squared angular momentum expectation value
!        for a three-dimensional wave function defined on a regular 
!        Cartesian grid using the rectangle rule and 2nd-order central
!        differences.
!
!     evua_ly2_3d_rect_cd4 is a PRIVATE, REAL scalar function that computes
!        the y-component of the squared angular momentum expectation value
!        for a three-dimensional wave function defined on a regular 
!        Cartesian grid using the rectangle rule and 4th-order central
!        differences.
!
!     evua_lz2_3d_rect_cd2 is a PRIVATE, REAL scalar function that computes
!        the z-component of the squared angular momentum expectation value
!        for a three-dimensional wave function defined on a regular 
!        Cartesian grid using the rectangle rule and 2nd-order central
!        differences.
!
!     evua_lz2_3d_rect_cd4 is a PRIVATE, REAL scalar function that computes
!        the z-component of the squared angular momentum expectation value
!        for a three-dimensional wave function defined on a regular 
!        Cartesian grid using the rectangle rule and 4th-order central
!        differences.
!
!     evua_fx_3d_rect_cd2 is a PRIVATE, REAL scalar function that computes
!        the x-component of the force expectation value for a 
!        three-dimensional wave function defined on a regular Cartesian
!        grid and acted upon by an applied external potential. It utilizes
!        the rectangle rule and 2nd-order central differences.
!
!     evua_fx_3d_rect_cd4 is a PRIVATE, REAL scalar function that computes
!        the x-component of the force expectation value for a 
!        three-dimensional wave function defined on a regular Cartesian
!        grid and acted upon by an applied external potential. It utilizes
!        the rectangle rule and 4th-order central differences.
!
!     evua_fy_3d_rect_cd2 is a PRIVATE, REAL scalar function that computes
!        the y-component of the force expectation value for a 
!        three-dimensional wave function defined on a regular Cartesian
!        grid and acted upon by an applied external potential. It utilizes
!        the rectangle rule and 2nd-order central differences.
!
!     evua_fy_3d_rect_cd4 is a PRIVATE, REAL scalar function that computes
!        the y-component of the force expectation value for a 
!        three-dimensional wave function defined on a regular Cartesian
!        grid and acted upon by an applied external potential. It utilizes
!        the rectangle rule and 4th-order central differences. 
!
!     evua_fz_3d_rect_cd2 is a PRIVATE, REAL scalar function that computes
!        the z-component of the force expectation value for a 
!        three-dimensional wave function defined on a regular Cartesian
!        grid and acted upon by an applied external potential. It utilizes
!        the rectangle rule and 2nd-order central differences.
!
!     evua_fz_3d_rect_cd4 is a PRIVATE, REAL scalar function that computes
!        the z-component of the force expectation value for a 
!        three-dimensional wave function defined on a regular Cartesian
!        grid and acted upon by an applied external potential. It utilizes
!        the rectangle rule and 4th-order central differences. 
!
!     evua_taux_3d_rect_cd2 is a PRIVATE, REAL scalar function that computes
!        the x-component of the torque expectation value due to an applied
!        external potential acting on a three-dimensional wave function 
!        defined on a regular Cartesian grid using the rectangle rule and
!        2nd-order central differences.
!
!     evua_taux_3d_rect_cd4 is a PRIVATE, REAL scalar function that computes
!        the x-component of the torque expectation value due to an applied
!        external potential acting on a three-dimensional wave function 
!        defined on a regular Cartesian grid using the rectangle rule and
!        4th-order central differences.
!
!     evua_tauy_3d_rect_cd2 is a PRIVATE, REAL scalar function that computes
!        the y-component of the torque expectation value due to an applied
!        external potential acting on a three-dimensional wave function 
!        defined on a regular Cartesian grid using the rectangle rule and
!        2nd-order central differences.
!
!     evua_tauy_3d_rect_cd4 is a PRIVATE, REAL scalar function that computes
!        the y-component of the torque expectation value due to an applied
!        external potential acting on a three-dimensional wave function 
!        defined on a regular Cartesian grid using the rectangle rule and
!        4th-order central differences.
!
!     evua_tauz_3d_rect_cd2 is a PRIVATE, REAL scalar function that computes
!        the z-component of the torque expectation value due to an applied
!        external potential acting on a three-dimensional wave function 
!        defined on a regular Cartesian grid using the rectangle rule and
!        2nd-order central differences.
!
!     evua_tauz_3d_rect_cd4 is a PRIVATE, REAL scalar function that computes
!        the z-component of the torque expectation value due to an applied
!        external potential acting on a three-dimensional wave function 
!        defined on a regular Cartesian grid using the rectangle rule and
!        4th-order central differences.
!
!     evua_ixx_3d_rect is a PRIVATE, REAL scalar function that computes
!        the expectation value of the xx-component of the moment of inertia 
!        tensor for a three-dimensional wave function defined on a regular
!        Cartesian grid using the rectangle rule.
!
!     evua_iyy_3d_rect is a PRIVATE, REAL scalar function that computes
!        the expectation value of the yy-component of the moment of inertia 
!        tensor for a three-dimensional wave function defined on a regular
!        Cartesian grid using the rectangle rule. 
!
!     evua_izz_3d_rect is a PRIVATE, REAL scalar function that computes 
!        the expectation value of the zz-component of the moment of inertia 
!        tensor for a three-dimensional wave function defined on a regular
!        Cartesian grid using the rectangle rule. 
!
!     evua_ixy_3d_rect is a PRIVATE, REAL scalar function that computes
!        the expectation value of the xy-component of the moment of inertia 
!        tensor for a three-dimensional wave function defined on a regular
!        Cartesian grid using the rectangle rule. 
!
!     evua_iyz_3d_rect is a PRIVATE, REAL scalar function that computes 
!        the expectation value of the yz-component of the moment of inertia 
!        tensor for a three-dimensional wave function defined on a regular
!        Cartesian grid using the rectangle rule. 
!
!     evua_ixz_3d_rect is a PRIVATE, REAL scalar function that computes 
!        the expectation value of the xz-component of the moment of inertia 
!        tensor for a three-dimensional wave function defined on a regular
!        Cartesian grid using the rectangle rule. 
! 
!     evua_vex_3d_rect is a PRIVATE, REAL scalar function that computes 
!        the potential energy expectation value due to an applied external
!        potential acting on a three-dimensional wave function defined on
!        a regular Cartesian grid using the rectangle rule.
!
!     evua_vmf_3d_rect is a PRIVATE, REAL scalar function that computes
!        the potential energy expectation value due to the mean-field
!        interactions between atoms in a Bose-Einstein condensate for a
!        three-dimensional condensate wave function defined on a regular
!        Cartesian grid using the rectangle rule.
!
! -------------------------------------------------------------------------
...
! -------------------------------------------------------------------------
      ...
      IF ( evuaQuadRule == 1 ) THEN
         ...
         temp = evua_r2_xy_3d_rect ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , xO , yO , dX , dY , dZ , X , Y , Psi3 )
         CALL MPI_REDUCE ( temp , evuaR2a , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

         temp = evua_r2_xy_3d_rect ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , evuaX , evuaY , dX , dY , dZ , X , Y , Psi3 )
         ...
         temp = evua_ixx_3d_rect ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , evuaY , evuaZ , dX , dY , dZ , Y , Z , Psi3 )
         ...
         temp = evua_ixy_3d_rect ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , evuaX , evuaY , dX , dY , dZ , X , Y , Psi3 )
         ...
         temp = evua_ixz_3d_rect ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , evuaX , evuaZ , dX , dY , dZ , X , Z , Psi3 )
         ...
         temp = evua_iyy_3d_rect ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , evuaX , evuaZ , dX , dY , dZ , X , Z , Psi3 )
         ...
         temp = evua_iyz_3d_rect ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , evuaY , evuaZ , dX , dY , dZ , Y , Z , Psi3 )
         ...
         temp = evua_izz_3d_rect ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , evuaX , evuaY , dX , dY , dZ , X , Y , Psi3 )
         ...
         IF ( evuaFdOrder == 2 ) THEN
            ...
            temp = evua_lx_3d_rect_cd2 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , yO , zO ,  dX , dY , dZ , Y , Z , Psi3 )
            CALL MPI_REDUCE ( temp , evuaLxA , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_lx_3d_rect_cd2 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , evuaY , evuaZ ,  dX , dY , dZ , Y , Z , Psi3 )
            CALL MPI_REDUCE ( temp , evuaLxB , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_ly_3d_rect_cd2 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , xO , zO , dX , dY , dZ , X , Z , Psi3 )
            CALL MPI_REDUCE ( temp , evuaLyA , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_ly_3d_rect_cd2 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , evuaX , evuaZ , dX , dY , dZ , X , Z , Psi3 )
            CALL MPI_REDUCE ( temp , evuaLyB , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_lz_3d_rect_cd2 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , xO , yO , dX , dY , dZ , X , Y , Psi3 )
            CALL MPI_REDUCE ( temp , evuaLzA , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_lz_3d_rect_cd2 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , evuaX , evuaY , dX , dY , dZ , X , Y , Psi3 )
            CALL MPI_REDUCE ( temp , evuaLzB , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_lx2_3d_rect_cd2 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , yO , zO , dX , dY , dZ , Y , Z , Psi3 )
            CALL MPI_REDUCE ( temp , evuaLx2a , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_lx2_3d_rect_cd2 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , evuaY , evuaZ , dX , dY , dZ , Y , Z , Psi3 )
            CALL MPI_REDUCE ( temp , evuaLx2b , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_ly2_3d_rect_cd2 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , xO , zO , dX , dY , dZ , X , Z , Psi3 )
            CALL MPI_REDUCE ( temp , evuaLy2a , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_ly2_3d_rect_cd2 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , evuaX , evuaZ , dX , dY , dZ , X , Z , Psi3 )
            CALL MPI_REDUCE ( temp , evuaLy2b , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_lz2_3d_rect_cd2 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , xO , yO , dX , dY , dZ , X , Y , Psi3 )
            CALL MPI_REDUCE ( temp , evuaLz2a , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_lz2_3d_rect_cd2 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , evuaX , evuaY , dX , dY , dZ , X , Y , Psi3 )
            ...
            temp = evua_taux_3d_rect_cd2 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , yO , zO , dX , dY , dZ , Y , Z , Vex3 , Psi3 )
            CALL MPI_REDUCE ( temp , evuaTauXa , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_taux_3d_rect_cd2 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , evuaY , evuaZ , dX , dY , dZ , Y , Z , Vex3 , Psi3 )
            CALL MPI_REDUCE ( temp , evuaTauXb , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_tauy_3d_rect_cd2 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , xO , zO , dX , dY , dZ , X , Z , Vex3 , Psi3 )
            CALL MPI_REDUCE ( temp , evuaTauYa , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_tauy_3d_rect_cd2 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , evuaX , evuaZ , dX , dY , dZ , X , Z , Vex3 , Psi3 )
            CALL MPI_REDUCE ( temp , evuaTauYb , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_tauz_3d_rect_cd2 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , xO , yO , dX , dY , dZ , X , Y , Vex3 , Psi3 )
            CALL MPI_REDUCE ( temp , evuaTauZa , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_tauz_3d_rect_cd2 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , evuaX , evuaY , dX , dY , dZ , X , Y , Vex3 , Psi3 )
         ...
         ELSE IF ( evuaFdOrder == 4 ) THEN
            ...
            temp = evua_lx_3d_rect_cd4 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , yO , zO ,  dX , dY , dZ , Y , Z , Psi3 )
            CALL MPI_REDUCE ( temp , evuaLxA , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_lx_3d_rect_cd4 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , evuaY , evuaZ ,  dX , dY , dZ , Y , Z , Psi3 )
            CALL MPI_REDUCE ( temp , evuaLxB , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_ly_3d_rect_cd4 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , xO , zO , dX , dY , dZ , X , Z , Psi3 )
            CALL MPI_REDUCE ( temp , evuaLyA , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_ly_3d_rect_cd4 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , evuaX , evuaZ , dX , dY , dZ , X , Z , Psi3 )
            CALL MPI_REDUCE ( temp , evuaLyB , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_lz_3d_rect_cd4 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , xO , yO , dX , dY , dZ , X , Y , Psi3 )
            CALL MPI_REDUCE ( temp , evuaLzA , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_lz_3d_rect_cd4 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , evuaX , evuaY , dX , dY , dZ , X , Y , Psi3 )
            CALL MPI_REDUCE ( temp , evuaLzB , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_lx2_3d_rect_cd4 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , yO , zO , dX , dY , dZ , Y , Z , Psi3 )
            CALL MPI_REDUCE ( temp , evuaLx2a , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_lx2_3d_rect_cd4 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , evuaY , evuaZ , dX , dY , dZ , Y , Z , Psi3 )
            CALL MPI_REDUCE ( temp , evuaLx2b , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_ly2_3d_rect_cd4 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , xO , zO , dX , dY , dZ , X , Z , Psi3 )
            CALL MPI_REDUCE ( temp , evuaLy2a , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_ly2_3d_rect_cd4 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , evuaX , evuaZ , dX , dY , dZ , X , Z , Psi3 )
            CALL MPI_REDUCE ( temp , evuaLy2b , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_lz2_3d_rect_cd4 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , xO , yO , dX , dY , dZ , X , Y , Psi3 )
            CALL MPI_REDUCE ( temp , evuaLz2a , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_lz2_3d_rect_cd4 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , evuaX , evuaY , dX , dY , dZ , X , Y , Psi3 )
            CALL MPI_REDUCE ( temp , evuaLz2b , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )
            ...
            temp = evua_taux_3d_rect_cd4 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , yO , zO , dX , dY , dZ , Y , Z , Vex3 , Psi3 )
            CALL MPI_REDUCE ( temp , evuaTauXa , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_taux_3d_rect_cd4 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , evuaY , evuaZ , dX , dY , dZ , Y , Z , Vex3 , Psi3 )
            CALL MPI_REDUCE ( temp , evuaTauXb , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_tauy_3d_rect_cd4 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , xO , zO , dX , dY , dZ , X , Z , Vex3 , Psi3 )
            CALL MPI_REDUCE ( temp , evuaTauYa , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_tauy_3d_rect_cd4 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , evuaX , evuaZ , dX , dY , dZ , X , Z , Vex3 , Psi3 )
            CALL MPI_REDUCE ( temp , evuaTauYb , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_tauz_3d_rect_cd4 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , xO , yO , dX , dY , dZ , X , Y , Vex3 , Psi3 )
            CALL MPI_REDUCE ( temp , evuaTauZa , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )

            temp = evua_tauz_3d_rect_cd4 ( nXa , nXb , nXbc , nYa , nYb , nYbc , nZa , nZb , nZbc , evuaX , evuaY , dX , dY , dZ , X , Y , Vex3 , Psi3 )
            CALL MPI_REDUCE ( temp , evuaTauZb , 1 , mpiReal , MPI_SUM , mpiMaster , MPI_COMM_WORLD , mpiError )
